...
SCDLLName("VH/VL Multi-Timeframe Scanner V2 (with Dashboard)")
...
struct s_TimeframeScanner {
    ...
    float VH_AnchorHigh = 0.0f;       // HIGH of current anchor (for new anchor check)
    float VH_ConfirmLevel = 0.0f;     // LOW of current anchor (for confirmation)
    ...
    float VL_AnchorLow = 0.0f;        // LOW of current anchor (for new anchor check)
    float VL_ConfirmLevel = 0.0f;     // HIGH of current anchor (for confirmation)
    ...
    // For higher timeframes: tracking
    ...
    // For Higher TF: bar counter (om correcte barIndex te hebben)
    int TFBarCounter = 0;
};
...
struct s_HigherTFBar {
    ...
    // Previous bar (for prev_high/prev_low)
    float PrevHigh = 0.0f;
    float PrevLow = 0.0f;
    float PrevClose = 0.0f;
    
    // NEW: Track the START 15s bar of the anchor bar (for confirm level drawing)
...
void ScanHigherTFBar(
    ...
    // CORRECTION: Use TFBarCounter for consistent tracking
    int barIndex = scanner->TFBarCounter;
    scanner->TFBarCounter++;  // Increment for next bar
    ...
    // For first bar: use current bar as reference
    ...
    if (vh_confirmed) {
        ...
        if (exact15sBar >= 0) {
            ...
            // NEW: Start VL search range from THIS plot (don't wait for VL search start)
            scanner->VL_StartBar_15s = exact15sBar;
            
            SCString msg;
            msg.Format("[%s] *** VH PLOTTED *** 15s bar %d (TF bar %d), TF Close=%.2f < ConfirmLevel=%.2f, Peak=%.2f, Scanned range %d-%d",
    ...
    if (vl_confirmed) {
        ...
        if (exact15sBar >= 0) {
            ...
            // NEW: Start VH search range from THIS plot (don't wait for VH search start)
            scanner->VH_StartBar_15s = exact15sBar;
            
            SCString msg;
            msg.Format("[%s] *** VL PLOTTED *** 15s bar %d (TF bar %d), TF Close=%.2f > ConfirmLevel=%.2f, Trough=%.2f, Scanned range %d-%d",
    ...
    if (!scanner->VH_Active) {
        ...
        if (close > prev_high) {
            ...
            scanner->VH_AnchorHigh = high;        // HIGH of first anchor
            scanner->VH_ConfirmLevel = low;       // LOW of first anchor
            ...
            SCString msg;
            msg.Format("[%s] *** VH SEARCH STARTED *** TFBar %d, AnchorHigh=%.2f, ConfirmLevel=%.2f (LOW), Close(%.2f)>PrevHigh(%.2f)",
        ...
        // Peak: always update on higher high
        ...
        // ANCHOR update: only when bodyclose > AnchorHigh!
        if (close > scanner->VH_AnchorHigh) {
            scanner->VH_AnchorHigh = high;        // HIGH of new anchor
            scanner->VH_ConfirmLevel = low;       // LOW of new anchor
            scanner->VH_ConfirmLevelBar = barIndex;
            scanner->VH_ConfirmLevelBar_15s = tfBar->LowBar_15s;
            anchorUpdated = true;
        }
        ...
        if (detailedLog && (peakUpdated || anchorUpdated)) {
            ...
            if (peakUpdated && anchorUpdated) {
                msg.Format("[%s]   VH UPDATED: TFBar %d, NewPeak=%.2f, NewAnchor(High=%.2f, ConfirmLevel=%.2f)",
                    tfName, barIndex, high, high, low);
            } else if (peakUpdated) {
                msg.Format("[%s]   VH PEAK UPDATED: TFBar %d, NewPeak=%.2f (Anchor unchanged)", tfName, barIndex, high);
            } else {
                msg.Format("[%s]   VH ANCHOR UPDATED: TFBar %d, Close(%.2f)>AnchorHigh(%.2f), NewConfirmLevel=%.2f",
    ...
    if (!scanner->VL_Active) {
        ...
        if (close < prev_low) {
            ...
            scanner->VL_AnchorLow = low;          // LOW of first anchor
            scanner->VL_ConfirmLevel = high;      // HIGH of first anchor
            ...
            SCString msg;
            msg.Format("[%s] *** VL SEARCH STARTED *** TFBar %d, AnchorLow=%.2f, ConfirmLevel=%.2f (HIGH), Close(%.2f)<PrevLow(%.2f)",
        ...
        // Trough: always update on lower low
        ...
        // ANCHOR update: only when bodyclose < AnchorLow!
        if (close < scanner->VL_AnchorLow) {
            scanner->VL_AnchorLow = low;          // LOW of new anchor
            scanner->VL_ConfirmLevel = high;      // HIGH of new anchor
            scanner->VL_ConfirmLevelBar = barIndex;
            scanner->VL_ConfirmLevelBar_15s = tfBar->HighBar_15s;
            anchorUpdated = true;
        }
        ...
        if (detailedLog && (troughUpdated || anchorUpdated)) {
            ...
            if (troughUpdated && anchorUpdated) {
                msg.Format("[%s]   VL UPDATED: TFBar %d, NewTrough=%.2f, NewAnchor(Low=%.2f, ConfirmLevel=%.2f)",
                ...
                msg.Format("[%s]   VL ANCHOR UPDATED: TFBar %d, Close(%.2f)<AnchorLow(%.2f), NewConfirmLevel=%.2f",
...
SCSFExport scsf_VHVLScanner_MultiTF(SCStudyInterfaceRef sc)
{
    ...
    SCInputRef i_ATR_ShowMultiplier = sc.Input[71];
    ...
    if (sc.SetDefaults)
    {
        sc.GraphName = "VH/VL Multi-Timeframe Scanner V2";
        sc.AutoLoop = 1;
        sc.GraphRegion = 0;
        sc.UpdateAlways = 1;
        ...
        i_DashboardBgColor.SetColor(RGB(0, 0, 0));  // Black
        ...
        if (barToProcess > p_15s->LastProcessedBar) {
            ...
            if (!p_15s->VH_Active) {
                ...
                if (close > prev_high) {
                    ...
                    p_15s->VH_AnchorHigh = high;      // HIGH of first anchor
                    p_15s->VH_ConfirmLevel = low;     // LOW of first anchor
                    p_15s->VH_ConfirmLevelBar = barToProcess;
                    
                    if (i_DetailedLog.GetYesNo()) {
                        SCString msg;
                        msg.Format("[15s] VH SEARCH STARTED at bar %d: AnchorHigh=%.2f, ConfirmLevel=%.2f (LOW), Close(%.2f)>PrevHigh(%.2f)",
                ...
                if (close > p_15s->VH_AnchorHigh) {
                    p_15s->VH_AnchorHigh = high;      // HIGH of new anchor
                    p_15s->VH_ConfirmLevel = low;     // LOW of new anchor
                    p_15s->VH_ConfirmLevelBar = barToProcess;
                    anchorUpdated = true;
                }
                ...
                if (i_DetailedLog.GetYesNo() && (peakUpdated || anchorUpdated)) {
                    SCString msg;
                    if (peakUpdated && anchorUpdated) {
                        msg.Format("[15s] VH UPDATED at bar %d: NewPeak=%.2f, NewAnchor(High=%.2f, ConfirmLevel=%.2f)",
                            barToProcess, high, high, low);
                    } else if (peakUpdated) {
                        msg.Format("[15s] VH PEAK UPDATED at bar %d: NewPeak=%.2f (Anchor unchanged)", barToProcess, high);
                    } else {
                        msg.Format("[15s] VH ANCHOR UPDATED at bar %d: Close(%.2f)>AnchorHigh(%.2f), NewConfirmLevel=%.2f",
            ...
            if (!p_15s->VL_Active) {
                ...
                if (close < prev_low) {
                    ...
                    p_15s->VL_AnchorLow = low;        // LOW of first anchor
                    p_15s->VL_ConfirmLevel = high;    // HIGH of first anchor
                    p_15s->VL_ConfirmLevelBar = barToProcess;
                    
                    if (i_DetailedLog.GetYesNo()) {
                        SCString msg;
                        msg.Format("[15s] VL SEARCH STARTED at bar %d: AnchorLow=%.2f, ConfirmLevel=%.2f (HIGH), Close(%.2f)<PrevLow(%.2f)",
                ...
                if (close < p_15s->VL_AnchorLow) {
                    p_15s->VL_AnchorLow = low;        // LOW of new anchor
                    p_15s->VL_ConfirmLevel = high;    // HIGH of new anchor
                    p_15s->VL_ConfirmLevelBar = barToProcess;
                    anchorUpdated = true;
                }
                ...
                if (i_DetailedLog.GetYesNo() && (troughUpdated || anchorUpdated)) {
                    SCString msg;
                    if (troughUpdated && anchorUpdated) {
                        msg.Format("[15s] VL UPDATED at bar %d: NewTrough=%.2f, NewAnchor(Low=%.2f, ConfirmLevel=%.2f)",
                            barToProcess, low, low, high);
                    } else if (troughUpdated) {
                        msg.Format("[15s] VL TROUGH UPDATED at bar %d: NewTrough=%.2f (Anchor unchanged)", barToProcess, low);
                    } else {
                        msg.Format("[15s] VL ANCHOR UPDATED at bar %d: Close(%.2f)<AnchorLow(%.2f), NewConfirmLevel=%.2f",
    ...
    if (i_5m_Enabled.GetYesNo()) {
        ...
        if (ShouldStartNewBar(currentBarTime, 5, p_5m_CurrentBar->StartTime)) {
            // A new 5-min bar starts - log this once
            SCString msg;
    ...
    if (i == sc.ArraySize - 1 && i_DashboardEnabled.GetYesNo()) {
        ...
        // 15-SEC Status
        if (i_15s_Enabled.GetYesNo()) {
            const char* status = (p_15s->LastPlottedType == s_TimeframeScanner::LAST_VH) ? "VH ↑" : 
                                (p_15s->LastPlottedType == s_TimeframeScanner::LAST_VL) ? "VL ↓" : "--";
        ...
        // 1-MIN Status
        if (i_1m_Enabled.GetYesNo()) {
            const char* status = (p_1m->LastPlottedType == s_TimeframeScanner::LAST_VH) ? "VH ↑" : 
                                (p_1m->LastPlottedType == s_TimeframeScanner::LAST_VL) ? "VL ↓" : "--";
            
            // Calculate countdown
            int secondsInBar = 60;  // 1 minute
            SCDateTime currentTime = sc.BaseDateTimeIn[i];
            int currentSecond = currentTime.GetSecond();
            int secondsRemaining = secondsInBar - currentSecond;
            int mins = secondsRemaining / 60;
            int secs = secondsRemaining % 60;
            
            SCString line;
            line.Format("1MIN:  %s   [%02d:%02d]\n", status, mins, secs);
            dashboardText += line;
        }
        
        // 5-MIN Status
        if (i_5m_Enabled.GetYesNo()) {
            const char* status = (p_5m->LastPlottedType == s_TimeframeScanner::LAST_VH) ? "VH ↑" : 
                                (p_5m->LastPlottedType == s_TimeframeScanner::LAST_VL) ? "VL ↓" : "--";
            
            // Calculate countdown
            SCDateTime currentTime = sc.BaseDateTimeIn[i];
            int currentMinute = currentTime.GetMinute();
            int currentSecond = currentTime.GetSecond();
            int minuteInInterval = currentMinute % 5;  // 0-4
            int secondsElapsed = (minuteInInterval * 60) + currentSecond;
            int secondsRemaining = (5 * 60) - secondsElapsed;
            int mins = secondsRemaining / 60;
            int secs = secondsRemaining % 60;
            
            SCString line;
            line.Format("5MIN:  %s   [%02d:%02d]\n", status, mins, secs);
            dashboardText += line;
        }
        
        // 15-MIN Status
        if (i_15m_Enabled.GetYesNo()) {
            const char* status = (p_15m->LastPlottedType == s_TimeframeScanner::LAST_VH) ? "VH ↑" : 
                                (p_15m->LastPlottedType == s_TimeframeScanner::LAST_VL) ? "VL ↓" : "--";
            
            // Calculate countdown
            SCDateTime currentTime = sc.BaseDateTimeIn[i];
            int currentMinute = currentTime.GetMinute();
            int currentSecond = currentTime.GetSecond();
            int minuteInInterval = currentMinute % 15;  // 0-14
            int secondsElapsed = (minuteInInterval * 60) + currentSecond;
            int secondsRemaining = (15 * 60) - secondsElapsed;
            int mins = secondsRemaining / 60;
            int secs = secondsRemaining % 60;
            
            SCString line;
            line.Format("15MIN: %s   [%02d:%02d]\n", status, mins, secs);
            dashboardText += line;
        }
        ...
        tool.FontBackColor = i_DashboardBgColor.GetColor();
        tool.FontSize = i_DashboardFontSize.GetInt();
        tool.FontBold = 1;
        tool.AddMethod = UTAM_ADD_OR_ADJUST;
